...
const CATEGORY = "Ainotate";
const INSTALL_URL = "https://ainotate.ai/docs/getting-started/installation/";
...
let runtimePromise: Promise<AinotateRuntime> | null = null;

export default function ainotateAmpPlugin(amp: PluginAPI) {
  amp.logger.log("[ainotate] Amp plugin initialized");
  ...
  amp.registerCommand(
    "ainotate-review",
    {
      ...
      category: CATEGORY,
      description: "Open Ainotate code review for the current workspace changes.",
    },
    async (ctx) => {
      const result = await runAinotate(amp, ctx, ["review"]);
      await handleReviewResult(ctx, result);
    },
  );

  amp.registerCommand(
    "ainotate-review-target",
    {
      title: "Review changes or PR",
      category: CATEGORY,
      description: "Open Ainotate code review for local changes, a PR/MR URL, or review arguments.",
    },
    async (ctx) => {
      ...
      const result = await runAinotate(amp, ctx, ["review", ...reviewArgs]);
      await handleReviewResult(ctx, result);
    },
  );
  ...
  amp.registerCommand(
    "ainotate-annotate",
    {
      title: "Annotate file",
      category: CATEGORY,
      description: "Open Ainotate annotation UI for a markdown/html file, folder, or URL.",
    },
    async (ctx) => {
      const target = await ctx.ui.input({
        title: "Annotate",
      ...
      const result = await runAinotate(amp, ctx, ["annotate", ...args, "--json"]);
  ...
  amp.registerCommand(
    "ainotate-last",
    {
      ...
      category: CATEGORY,
      description: "Open Ainotate annotation UI for Amp's latest assistant message.",
    },
    async (ctx) => {
      if (!ctx.thread) {
      ...
      const runtime = await getAinotateRuntime();
      let tempFile: string | null = null;
      let result: RunResult;

      try {
        if (runtime.features.stdinLast) {
          result = await runAinotate(
            amp,
            ctx,
          ...
          tempFile = join(tmpdir(), `ainotate-amp-last-${process.pid}-${Date.now()}-${randomUUID()}.md`);
          writeFileSync(tempFile, message, "utf8");
          result = await runAinotate(amp, ctx, ["annotate", tempFile, "--json"], { runtime });
        }
      } finally {
        if (tempFile) {
          try {
...
export function formatAnnotationFeedback(
  ...
  const config = loadAinotateConfig();
...
async function appendFeedback(ctx: CommandContext, content: string): Promise<void> {
  if (!ctx.thread) {
    await ctx.ui.notify("Ainotate produced feedback, but there is no active Amp thread.");
    return;
  }
...
async function notifyFailure(
  ...
  await ctx.ui.notify(`Ainotate ${mode} failed.${details ? `\n\n${details}` : ""}${installHint}`);
  return true;
}

async function runAinotate(
  amp: PluginAPI,
  ctx: CommandContext,
  args: string[],
  options: { stdin?: string; runtime?: AinotateRuntime } = {},
): Promise<RunResult> {
  const cwd = await resolveCwd(ctx);
  const runtime = options.runtime ?? await getAinotateRuntime();
  const readyFile = runtime.features.readyFile
    ? join(tmpdir(), `ainotate-amp-${process.pid}-${Date.now()}-${randomUUID()}.jsonl`)
    : null;
  const command = [...runtime.command, ...args];
  const env = buildEnv(buildAinotateEnv(cwd, readyFile));
  ...
  return {
    ...
    stdout,
    stderr,
    ...(readyTimedOut ? { error: "Timed out waiting for Ainotate to publish its browser URL." } : {}),
  };
}

export async function resolveCwd(ctx: CommandContext): Promise<string> {
  const explicitCwd = normalizeDirectory(process.env.AINOTATE_CWD);
...
export function buildAinotateEnv(cwd: string, readyFile: string | null): Record<string, string> {
  return {
    AINOTATE_ORIGIN: RUNTIME,
    AINOTATE_CWD: cwd,
    ...(readyFile ? { AINOTATE_READY_FILE: readyFile } : {}),
  };
}
...
async function notifyUrls(
  ...
  for (const rawUrl of matches) {
    ...
    seenUrls.add(url);
    await ctx.ui.notify(`Ainotate link:\n${url}`);
  }
}
...
async function getAinotateRuntime(): Promise<AinotateRuntime> {
  runtimePromise ??= resolveAinotateRuntime();
  return runtimePromise;
}

async function resolveAinotateRuntime(): Promise<AinotateRuntime> {
  const explicitSource = process.env.AINOTATE_AMP_SOURCE_ENTRY;
  const sourceEntry = explicitSource
    ? resolve(explicitSource)
    : process.env.AINOTATE_AMP_USE_SOURCE === "1"
  ...
  const { command, version } = resolveAinotateCommand();
  return {
    command,
    source: "cli",
    version,
...
function resolveAinotateCommand(): { command: string[]; version: string | null } {
  const candidates = getAinotateCommandCandidates();
  let fallback = candidates[candidates.length - 1] ?? ["ainotate"];
  ...
  for (const command of candidates) {
    ...
    if (isPathLike(executable)) {
      ...
      const version = detectAinotateVersion(command);
  ...
  return { command: fallback, version: detectAinotateVersion(fallback) };
}

export function getAinotateCommandCandidates(
  options: {
  ...
  const explicitBin = normalizeExecutablePath(env.AINOTATE_BIN);
  if (explicitBin) candidates.push([explicitBin]);

  if (platform === "win32") {
    const localAppData = normalizeExecutablePath(env.LOCALAPPDATA);
    if (localAppData) candidates.push([pathJoin(localAppData, "ainotate", "ainotate.exe")]);

    for (const home of homes) {
      candidates.push([pathJoin(home, ".local", "bin", "ainotate.exe")]);
    }
  } else {
    for (const home of homes) {
      candidates.push([pathJoin(home, ".local", "bin", "ainotate")]);
    }
  }

  candidates.push(["ainotate"]);
  ...
  while (true) {
    ...
    if (existsSync(packageJsonPath)) {
      try {
        ...
        if (pkg.name === "ainotate") return dir;
      } catch {
...
function detectAinotateVersion(command: string[]): string | null {
  try {
...
function loadAinotateConfig(): PromptConfig {
  try {
    const configPath = join(getAinotateDataDir(), "config.json");
...
export function getAinotateDataDir(): string {
  const value = process.env.AINOTATE_DATA_DIR?.trim();
  if (!value) return join(homedir(), ".ainotate");
