...
const helper = resolve(import.meta.dir, "ainotate-review");
const tempDirs: string[] = [];

function createHarness(delaySeconds = 0) {
  const root = mkdtempSync(join(tmpdir(), "ainotate-review-"));
  tempDirs.push(root);
  const binDir = join(root, "bin");
  const logPath = join(root, "launches.log");
  mkdirSync(binDir);
  const mockAinotate = join(binDir, "ainotate");
  writeFileSync(
    mockAinotate,
    `#!/bin/sh
printf '%s|%s\\n' "$AINOTATE_PORT" "\${AINOTATE_BROWSER:-}" >> "$HELPER_LOG"
sleep "\${HELPER_DELAY:-0}"
printf '%s\\n' '{"decision":{"approved":true}}'
`,
  );
  chmodSync(mockAinotate, 0o755);
  ...
  const env = {
    ...
    AINOTATE_DATA_DIR: join(root, "data"),
    AINOTATE_REVIEW_OWNER: "test-session",
  };
  return { env, logPath };
}
...
describe("ainotate-review", () => {
  ...
  test("continues a bounded wait without restarting the review", () => {
    ...
    const shortWaitEnv = { ...env, AINOTATE_POLL_BUDGET: "1" };

    const initial = invoke(["run", "review"], shortWaitEnv);
    expect(initial.exitCode).toBe(0);
    expect(initial.stdout.toString()).toContain("AINOTATE_STILL_WAITING");

    const resumed = invoke(["wait"], { ...env, AINOTATE_POLL_BUDGET: "5" });
